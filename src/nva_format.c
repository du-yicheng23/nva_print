 ...
 * 格式化所用的栈（用于保存待格式化的数据）
 */
static nva_Stack nva__fmt_stack = NVA_STACK_INIT_VALUE;
...
nva_FmtStatus nva_schar(const signed char value, const nva_FmtStatus status)
{
    ...
    if (nva_stackPush(&nva__fmt_stack, &value, NVA_TYPEID_SCHAR) == NVA_SUCCESS) {
        return NVA_START;
    }
...
nva_FmtStatus nva_uchar(const unsigned char uvalue, const nva_FmtStatus status)
{
    ...
    if (nva_stackPush(&nva__fmt_stack, &uvalue, NVA_TYPEID_UCHAR) == NVA_SUCCESS) {
        return NVA_START;
    }
...
nva_FmtStatus nva_short(const signed short value, const nva_FmtStatus status)
{
    ...
    if (nva_stackPush(&nva__fmt_stack, &value, NVA_TYPEID_SSHORT) == NVA_SUCCESS) {
        return NVA_START;
    }
...
nva_FmtStatus nva_ushort(const unsigned short uvalue, const nva_FmtStatus status)
{
    ...
    if (nva_stackPush(&nva__fmt_stack, &uvalue, NVA_TYPEID_USHORT) == NVA_SUCCESS) {
        return NVA_START;
    }
...
nva_FmtStatus nva_int(const int value, const nva_FmtStatus status)
{
    ...
    if (nva_stackPush(&nva__fmt_stack, &value, NVA_TYPEID_SINT) == NVA_SUCCESS) {
        return NVA_START;
    }
...
nva_FmtStatus nva_uint(const unsigned int uvalue, const nva_FmtStatus status)
{
    ...
    if (nva_stackPush(&nva__fmt_stack, &uvalue, NVA_TYPEID_UINT) == NVA_SUCCESS) {
        return NVA_START;
    }
...
nva_FmtStatus nva_ptr(const void* ptr, const nva_FmtStatus status)
{
    ...
    if (nva_stackPush(&nva__fmt_stack, &ptr, NVA_TYPEID_PTR) == NVA_SUCCESS) {
        return NVA_START;
    }
...
nva_FmtStatus nva_char(const char ch, const nva_FmtStatus status)
{
    ...
    if (nva_stackPush(&nva__fmt_stack, &ch, NVA_TYPEID_CHAR) == NVA_SUCCESS) {
        return NVA_START;
    }
 ...
 */
nva_ErrorCode nva_format(char* NVA_RESTRICT dest, const char* NVA_RESTRICT format, const nva_FmtStatus status)
{
    ...
    nva__fmt_stack.data_top = 0U;
    nva__fmt_stack.type_top = 0U;
    ...
    for (i = 0U, j = 0U; format[j] != '\0';) {
        ...
        if (in_formatting) {
            if (format[j] == '}') {
                ...
                if (style.arg_id == -1) {
                    error_code = nva_stackPeek(&nva__fmt_stack, stack_index, &current_phase_value.generic_v, &type_id);
                }
                else {
                    error_code = nva_stackPeek(&nva__fmt_stack, style.arg_id, &current_phase_value.generic_v, &type_id);
                }
